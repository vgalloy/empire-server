...
public class UserController {
  ...
  public UserController(
      ...
      final ControllerLinker controllerLinker) {
   ...
   */
  @GetMapping("{userId}")
  public ResourceData<UserDto> getById(
      @PathVariable @Valid @NotNull(message = USER_ID_MUST_BE_NOT_NULL) final UUID userId) {
    final var user =
        userService
            .getById(UserId.of(userId))
            .orElseThrow(() -> new NotFoundResourceException(userId));
   ...
   */
  @PostMapping
  public ResourceData<UserDto> create(
      @RequestBody @Valid @NotNull(message = USER_ID_MUST_BE_NOT_NULL) final UserDto userDto) {
   ...
   */
  @PutMapping("{userId}")
  public ResourceData<UserDto> update(
      @PathVariable @Valid @NotNull(message = USER_ID_MUST_BE_NOT_NULL) final UUID userId,
      @RequestBody @Valid @NotNull(message = "User can't be null") final UserDto userDto) {
  ...
  public ResourceData<UserDto> delete(
      @PathVariable @Valid @NotNull(message = USER_ID_MUST_BE_NOT_NULL) final UUID userId) {
  ...
  public ResourceList<UserDto> getAll() {
    final var users =
        userService.getAll().stream().map(userMapper::map).collect(Collectors.toList());
