...
public class EmpireController {
  ...
  public EmpireController(
      final EmpireMapper empireMapper,
      final EmpireService empireService,
      final EmpireDao empireDao,
   ...
   */
  @GetMapping("{empireId}")
  public ResourceData<EmpireDto> getById(
      @PathVariable @Valid @NotNull(message = "Empire id can't be null") final UUID empireId) {
    // EXTRACT
  ...
  public ResourceData<EmpireDto> nextRound(
      @PathVariable @Valid @NotNull(message = "Empire id can't be null") final UUID empireId) {
    // EXTRACT
    final var id = EmpireId.of(empireId);
    // DO
  ...
  public ResourceData<EmpireDto> updateOrder(
      @PathVariable @Valid @NotNull(message = "Empire id can't be null") final UUID empireId,
      @Valid @NotNull @RequestBody final Map<OrderType, Long> orders) {
    // EXTRACT
    final var id = EmpireId.of(empireId);
    // DO
    final var empire = empireService.getEmpireById(id);
