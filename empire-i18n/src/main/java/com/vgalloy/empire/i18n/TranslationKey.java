...
public class TranslationKey implements Comparable<TranslationKey> {
  ...
  private static final Comparator<TranslationKey> COMPARATOR =
      Comparator.comparing((TranslationKey key) -> key.locale.toLanguageTag())
  ...
  /**
   * According to BCP 47 specification {@link Locale} is divided into several part. This method
   * return a {@link TranslationKey key} with the parent of the current locale. Example : 'fr_FR'
   * will be 'fr'
   *
  ...
  public TranslationKey getParent() {
    ...
    final var newTagLang =
        Arrays.stream(tagLang).limit(tagLang.length - 1).collect(Collectors.joining("-"));
    return new TranslationKey(code, Locale.forLanguageTag(newTagLang));
  }

  /**
   * Return the current key and all parent order ascending. The first element of the list must this
   * the current key and the last must be the {@link #isDefault() default} key.
   *
