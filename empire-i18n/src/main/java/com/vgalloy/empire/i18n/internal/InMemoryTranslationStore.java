...
class InMemoryTranslationStore implements TranslationStore {
  ...
  private void run(final Translation translation, final UpdateStrategy updateStrategy) {
    translation
        ...
        .forEach(
            key -> {
              final Map.Entry<TranslationKey, String> currentTranslation =
                  new AbstractMap.SimpleEntry<>(key, this.translationTable.get(key));
              updateStrategy
                  .update(translation, currentTranslation)
  ...
  private interface UpdateStrategy {
    ...
    Optional<String> update(
        Translation newTranslation, Map.Entry<TranslationKey, String> currentTranslation);
  }
